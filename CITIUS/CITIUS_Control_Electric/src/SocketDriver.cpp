...
string SocketDriver::getValueFromConfig(string parameter, int device){
  ...
  if (device == DEVICE_ELECTRIC || device == DEVICE_DRIVING) {
    fichero.open("/home/atica/catkin_ws/src/CITIUS/CITIUS_Control_Electric/bin/socket.conf");
  } else if (device == DEVICE_XSENS) {
    fichero.open("/home/atica/catkin_ws/src/CITIUS/CITIUS_Control_Electric/bin/socket_INSGPS.conf");
  } else if (device == DEVICE_AHRS) {
    fichero.open("/home/atica/catkin_ws/src/CITIUS/CITIUS_Control_Electric/bin/socket_MAGN.conf");
  }
  if (!fichero.is_open()) {
    return "";
  }
