...
private:
    ...
    void fcn_receive_run_mission(OjCmpt, JausMessage);
    // Componente Primitive Driver
    void fcn_receive_set_wrench_effort(OjCmpt,JausMessage);
    void fcn_receive_set_discrete_devices(OjCmpt,JausMessage);
    // Componente Visual Sensor
    void fcn_receive_set_camera_pose(OjCmpt,JausMessage);
    void fcn_receive_set_signaling_elements(OjCmpt,JausMessage);
    void fcn_receive_set_positioner(OjCmpt,JausMessage);
    void fcn_receive_set_day_time_camera(OjCmpt,JausMessage);
    void fcn_receive_set_night_time_camera(OjCmpt,JausMessage);
    // Componente Velocity State Sensor
    void fcn_receive_set_travel_speed(OjCmpt,JausMessage);
    // Componente HeartBeat Information
    void fcn_receive_heartbeat_channel_state(OjCmpt,JausMessage);
    void fcn_receive_heartbeat_position_info(OjCmpt,JausMessage);
};